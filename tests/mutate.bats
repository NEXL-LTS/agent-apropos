...
# Real loop-control mutants insert a line rather than replacing one, so the
# reported text has to fold the span. Opt in to that shape.
...
# The real engine's failure modes, which the runner is required to catch.
if [ -n "${STUB_ENGINE_EXITS_NONZERO:-}" ]; then echo "boom" >&2; exit 3; fi
if [ -n "${STUB_ENGINE_RULES_MISSING:-}" ]; then
  # universalmutator prints this and exits 0, which is why the runner cannot
  # trust the exit status alone.
...
# The success marker the real engine always prints, and the runner now requires.
...
# A mutant that inserts a line rather than replacing one: the reported "now"
# text must carry both lines, and the "was" text must still be the single
# unmutated source line.
...
# The whole-suite fallback used to score its survivors as killed: the re-check
# asked whether a *wider* suite killed the mutant, and for a target that already
# was the whole suite that question has no wider answer, so control fell through
# to the killed branch. A module exempted from having a sibling spec was
# therefore ungated.
...
# The compile gate cannot build the spec directory: `crystal build` fails on it
# with "Is a directory", which the engine reads as "this mutant does not
# compile" — so every mutant would be discarded and the module left ungated.
...
# A gate that cannot tell "the engine did nothing" from "there was nothing to
# do" is a gate that reports green over zero work.
...
# The engine prints COULD NOT FIND RULE FILE and exits 0, so the exit status
# alone would let a broken rules file pass as a clean gate.
...
# The line set comes from parsing diff text, so any git config that changes that
# text changes what the gate mutates. An external driver would replace it
# wholesale and a prefix setting would move the path; both would leave the parser
# finding nothing, which looks exactly like a change that touched no source.
...
# Two paths that a flattened key would collide on. A collision here would
# restore one module's source over the other's.
